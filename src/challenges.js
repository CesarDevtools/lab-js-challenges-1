...
function filterOut(arr1, arr2) {
  ...
  if (arr1.length === 0) {
    ...
    for (let i = 0; i < arr1.length; i++) {
      let palabra = arr1[i];
      if (!arr2.includes(palabra)) {
        arrLimpio.push(arr1[i]);
      }
    }
  }
  return arrLimpio;
  ...
  for (let i = 0; i < 20; i++) {
    for (let j = 0; j < 20; j++) {
      ...
      if (productHorizontal > maxProduct) {
        ...
        maxNumbers = `The greatest product will be the ${arr[i][j]} x ${arr[i][j + 1]} x ${arr[i][j + 2]} x ${arr[i][j + 3]}`;
      }
      if (productVertical > maxProduct) {
        maxProduct = productVertical
        maxNumbers = `The greatest product will be the ${arr[i][j]} x ${arr[i + 1][j]} x ${arr[i + 2][j]} x ${arr[i + 3][j]}`;
      }
    }
  }
  return maxProduct;
